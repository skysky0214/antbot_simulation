...
3. The [`Estop_State`](/antbot/en/development-guide/control-table/#emergency-stop) on the RCU board changes to <Badge text="ON" variant="danger" />.
...
Driving is only possible after the motor state transitions through `IDLE` → `READY_ENTER` → `READY` (within 1 second). See [Control Table > Motor](/antbot/en/development-guide/control-table/#motor) for details.
:::
