...
:::note[ARM vs x86 빌드]
`antbot_camera`(`OrbbecSDK_ROS2`), `antbot_hw_interface`, `antbot_imu`, `vanjee_lidar_*` 등은 ARM 전용 라이브러리에 의존하므로 x86 환경에서는 컴파일되지 않습니다.
- **ARM (Jetson 등)**: `colcon build`로 전체 빌드 가능
- **x86 (일반 PC)**: 시뮬레이션에 필요한 패키지만 선택적으로 빌드

시뮬레이션에 필요한 패키지: `antbot_gazebo`, `antbot_navigation`, `antbot_teleop` (및 이들의 의존 패키지인 `antbot_description`, `antbot_swerve_controller`, `antbot_libs`, `antbot_interfaces`)

```bash
# x86 환경 — 시뮬레이션 관련 패키지만 빌드
colcon build --symlink-install \
  --packages-up-to antbot_gazebo antbot_navigation antbot_teleop
```
:::
...
gazebo.launch.py
  ...
  ├── xacro → URDF 생성
  ├── worlds.yaml로 월드 이름 → SDF 경로 해석
  ├── ign gazebo -r world.sdf
  ├── 로봇 스폰 (x=0, y=0, z=0.5)
  ├── robot_state_publisher
  ├── [spawn_robot 종료 대기 (OnProcessExit)]
  ├── 컨트롤러 상태 폴링 (unconfigured 대기)
  ├── joint_state_broadcaster configure + activate
  ├── swerve_drive_controller configure + activate
...
|------|-----------------|------|---------|------|--------|
| 전방 2D LiDAR | `gpu_lidar` | `/scan_0` | 720 | 0.6-20m | stddev 0.008 |
| 후방 2D LiDAR | `gpu_lidar` | `/scan_1` | 720 | 0.6-20m | stddev 0.008 |
| IMU | `imu_sensor` | `/imu/data` | — | — | 각속도 0.0003, 가속도 0.02 |
...
`antbot_navigation/maps/worlds.yaml`에 등록된 월드 이름 또는 SDF 파일 경로를 지정할 수 있습니다.
...
```yaml
# antbot_navigation/maps/worlds.yaml
worlds:
  depot:
    sdf: depot.sdf        # antbot_navigation/maps/ 기준 상대경로
    map: depot_sim.yaml    # Nav2에서 사용하는 맵 파일
```

새 월드를 추가하려면 이 파일에 항목을 추가하고, SDF 파일을 `antbot_navigation/maps/`에 배치하세요.
