...
컨트롤 테이블은 장치의 현재 상태와 구동 및 제어에 필요한 다수의 데이터로 이루어져 있습니다. 컨트롤 테이블을 구성하는 각각의 데이터를 **컨트롤 아이템**이라 하며, 각 아이템은 고유한 이름(Name), 크기, 접근권한 등의 속성을 가집니다.

사용자는 **[다이나믹셀 프로토콜 2.0](https://emanual.robotis.com/docs/kr/dxl/protocol2/)** 을 사용해 컨트롤 테이블의 특정 아이템을 읽어서 장치의 상태를 파악할 수 있고, 값을 변경함으로써 장치를 제어할 수 있습니다.
