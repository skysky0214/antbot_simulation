...
| 증상 | 원인 |
|------|------|
| "Failed to create plan" | 로봇이 코스트맵 장애물 내부에 위치 |
| 벽 충돌 / 장애물 회피 실패 | 안전 여유 공간 부족 |
| 제자리에서 과도한 회전 | 방향 제어 가중치 불균형 |
| map 프레임 없음 / TF 타임아웃 | Gazebo 재시작으로 sim time 리셋 |
...
- `inflation_radius` 증가 (0.75 → 1.0) — 장애물 주변 안전 영역 확대
- `cost_scaling_factor` 감소 (1.5 → 1.0) — 장애물 근처 비용을 더 높게 유지
- `ObstaclesCritic.collision_margin_distance` 증가 (0.1 → 0.2) — MPPI가 더 일찍 회피

### 제자리에서 과도한 회전

- `TwirlingCritic` 가중치 증가 (10.0 → 15.0) — 불필요한 회전 억제
- `wz_max` 감소 (1.5 → 1.0) — 최대 회전 속도 제한
- `PreferForwardCritic` 가중치 감소 (15.0 → 10.0) — 전진 방향 탐색 시간 단축
