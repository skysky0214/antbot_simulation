...
Nav2는 여러 컴포넌트가 협력하여 자율주행을 수행합니다. 각 컴포넌트의 역할은 다음과 같습니다:
...
:::note[time_steps와 예측 시간의 관계]
`time_steps × model_dt = 56 × 0.05초 = 2.8초`. MPPI는 현재 시점부터 2.8초 후까지의 로봇 경로를 예측합니다.
`time_steps`를 늘리면 더 먼 미래를 예측하지만 연산량이 증가하고, 줄이면 가까운 경로만 고려하여 반응이 빨라집니다.
:::

:::note[vy_max가 낮은 이유]
`vy_max`를 낮게 설정하면 MPPI가 옆으로 이동하는 경로를 거의 생성하지 않습니다.
스티어링이 ±60도까지만 꺾이므로, 횡이동을 시도하면 스티어링이 한계에 걸려 부자연스러운 움직임이 발생합니다.
:::

#### MPPI Critics (경로 평가 기준)

MPPI는 후보 경로마다 여러 평가 기준(Critic)으로 점수를 매깁니다. 가중치가 높을수록 해당 기준이 경로 선택에 더 큰 영향을 줍니다.
...
시뮬레이션에서는 마찰이 없어 로봇이 쉽게 미끄러지므로 가중치를 높게 설정합니다.
실제 로봇은 바퀴와 지면의 마찰이 자연스러운 직진을 유도하므로 가중치가 낮아도 됩니다.
:::

**경로 추종** — 목표 지점까지 경로를 잘 따라가도록 하는 설정:
...
:::note[충돌 보호]
벽 충돌 시 바퀴가 미끄러지면서 오도메트리에 비정상적으로 큰 속도값이 발생할 수 있습니다.
`odom0_rejection_threshold`가 이런 스파이크를 자동으로 무시하여, 충돌 후에도 위치 추정이 크게 틀어지지 않도록 보호합니다.
:::
...
:::caution["Failed to create plan" 오류]
코스트맵에서 로봇이 장애물 내부에 위치한 경우 발생합니다. RViz에서 **2D Pose Estimate**로 초기 위치를 수정하거나, 아래 명령어로 코스트맵을 초기화하세요:

```bash
# 글로벌 코스트맵을 초기화하여 장애물 정보를 재설정합니다
ros2 service call /global_costmap/clear_entirely_global_costmap nav2_msgs/srv/ClearEntireCostmap
```
:::

:::caution[벽 충돌 / 장애물 회피 실패]
로봇이 장애물에 너무 가깝게 접근하거나 충돌하는 경우:

| 조치 | 파라미터 | 변경 예시 | 효과 |
|------|----------|-----------|------|
| 여유 공간 확대 | `inflation_radius` | 0.75 → 1.0 | 장애물 주변 안전 영역 증가 |
| 비용 곡선 조정 | `cost_scaling_factor` | 1.5 → 1.0 | 장애물 근처 비용을 더 높게 유지 |
| MPPI 회피 거리 증가 | `ObstaclesCritic.collision_margin_distance` | 0.1 → 0.2 | MPPI가 장애물을 더 일찍 회피 |
:::

:::caution[로봇이 제자리에서 과도하게 회전]
목표 지점을 향해 이동하지 않고 계속 회전만 하는 경우:

| 조치 | 파라미터 | 변경 예시 | 효과 |
|------|----------|-----------|------|
| 회전 억제 강화 | `TwirlingCritic` 가중치 | 10.0 → 15.0 | 불필요한 회전에 더 큰 페널티 |
| 회전 속도 제한 | `wz_max` | 1.5 → 1.0 | 최대 회전 속도 감소 |
| 전진 유도 완화 | `PreferForwardCritic` 가중치 | 15.0 → 10.0 | 전진 방향 탐색 시간 단축 |
:::

:::caution[map 프레임 없음 / TF 타임아웃]
