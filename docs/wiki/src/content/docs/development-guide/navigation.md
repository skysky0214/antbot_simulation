...
| 증상 | 원인 | 해결 방법 |
|------|------|-----------|
| **"Failed to create plan"** | 코스트맵에서 로봇이 장애물 내부에 위치 | RViz에서 **2D Pose Estimate**로 초기 위치 수정, 또는 코스트맵 초기화 (아래 명령어) |
| **벽 충돌 / 장애물 회피 실패** | 안전 여유 공간 부족 | `inflation_radius` 증가 (0.75→1.0), `cost_scaling_factor` 감소 (1.5→1.0), `ObstaclesCritic.collision_margin_distance` 증가 (0.1→0.2) |
| **로봇이 제자리에서 과도하게 회전** | 방향 제어 가중치 불균형 | `TwirlingCritic` 증가 (10.0→15.0), `wz_max` 감소 (1.5→1.0), `PreferForwardCritic` 감소 (15.0→10.0) |
| **map 프레임 없음 / TF 타임아웃** | Gazebo 재시작으로 sim time 리셋 | **Gazebo와 Navigation을 항상 함께 재시작** |

```bash
# 코스트맵 초기화 ("Failed to create plan" 발생 시)
